    ...
    <header className="sticky top-0 z-30 h-16 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-6 flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 sm:w-88 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-200/90 p-3.5 z-50 animate-scale-in origin-top-right">
              <div className="flex items-center justify-between px-2 py-1.5 border-b border-slate-100 mb-2">
                <span className="font-bold text-xs text-slate-900 uppercase tracking-wider font-mono">Telemetry Notifications</span>
                ...
                {notifications.map((n) => (
                  <div
                    ...
                    className={`p-2.5 rounded-xl transition cursor-pointer ${
                      n.unread ? 'bg-indigo-50/60 hover:bg-indigo-50/90 border border-indigo-100' : 'hover:bg-slate-50 border border-transparent'
                    }`}
                  >
                    <div className="flex items-start gap-2.5">
                      <div className="w-2 h-2 rounded-full bg-indigo-600 mt-1 shrink-0 animate-pulse" />
        ...
        <div className="relative pl-2 border-l border-slate-200">
          <button
            ...
            className="flex items-center gap-2.5 p-1.5 rounded-xl hover:bg-slate-100/80 transition focus:outline-none btn-tactile cursor-pointer"
            aria-label="User menu"
          >
            <div className="relative">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-slate-900 to-indigo-950 text-white font-bold text-xs flex items-center justify-center border border-indigo-500/20 shadow-xs">
          ...
          {showUserMenu && (
            <div className="absolute right-0 mt-2 w-56 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-200/90 p-2 z-50 animate-scale-in origin-top-right">
              <div className="px-3 py-2.5 border-b border-slate-100 mb-1">
                <p className="text-xs font-bold text-slate-800">{userName}</p>
                <p className="text-[11px] text-slate-500 truncate font-mono">{user?.email || 'Demo Mode (Offline)'}</p>
              </div>
              <button
                onClick={() => {
                ...
                className="w-full text-left px-3 py-2 text-xs font-medium text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 rounded-xl transition cursor-pointer"
              >
                Account Settings
              </button>
              <button
                ...
                className="w-full text-left flex items-center gap-2 px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50 rounded-xl transition mt-1 cursor-pointer"
              >
